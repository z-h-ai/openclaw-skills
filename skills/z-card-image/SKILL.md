...
6. **聊天里直接发图**：生成后按「输出与发送规范」发送；图片优先落到工作区 `tmp/`
...
- 图片优先输出到工作区相对路径，如 `tmp/card.png`、`tmp/card_01.png`
- 优先使用 message tool 的 `path` / `filePath`
- 如果必须内联，只使用 `MEDIA:./tmp/card.png` 这种相对路径
- 不要使用 `MEDIA:/root/...` 或 `MEDIA:~...`
- 若图片先生成在工作区外，先复制到 `tmp/` 再发送
- 渲染脚本若已经输出了 `MEDIA:./tmp/...`，直接沿用，不要改写成绝对路径
